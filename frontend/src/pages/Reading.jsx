...
const Reading = () => {
  ...
  // Count a page as read if stayed on for at least 3 seconds
  useEffect(() => {
    if (!isTiming || !book) return;
    
    const pageToCount = currentPage;
    const timer = setTimeout(() => {
        if (!countedPagesRef.current.has(pageToCount)) {
            countedPagesRef.current.add(pageToCount);
            setPagesReadDuringSession(prev => prev + 1);
        }
    }, 3000);
